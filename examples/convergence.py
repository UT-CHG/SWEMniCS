...
def run_experiment(name):
    ...
    prob = ConvergenceProblem(dt=dt,nt=nt,nx=nx,ny=ny,friction_law=fric_law,solution_var=sol_var,spherical=False)
    p_degree = [1,1]
    rel_toleran=1e-11
    abs_toleran=4e-12
    max_iter=10
    ...
    zeta_exact=np.real(alpha_0*np.exp(1j*omega*t)*((np.cos(beta*(x_pt)))/(np.cos(beta*xL))))
    vel_exact=np.real(-1j*omega*alpha_0/(beta*H0)*np.exp(1j*omega*t)*(np.sin(beta*(x_pt)))/(np.cos(beta*(xL))) )
    #overwrite
    ...
    solver.time_loop(solver_parameters=params,stations=stations,plot_every=plot_each,plot_name='Kubatko_test_'+name)
    ...
    if rank ==0:
        ...
        plt.plot(t, solver.vals[:,:,0].flatten(), "--", linewidth=2, label="h at "+str(x_pt))
        plt.plot(t,zeta_exact,'k',label="Exact solution")
        ...
        plt.plot(np.linspace(0,t_f/(60*60*24),nt+1), solver.vals[:,:,1].flatten(), "--", linewidth=2, label="v at"+str(x_pt))
        #plt.plot(points_on_proc[:, 1], p_values, "b--", linewidth = 2, label="Load")
        plt.grid(True)
        plt.xlabel("t(day)")
