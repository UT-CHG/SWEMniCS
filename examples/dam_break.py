...
def run_experiment(name, outdir=None, **kwargs):
    ...
    solver.time_loop(solver_parameters=params,stations=stations,plot_every=1,plot_name='dam_test_'+name)
    ...
    if rank ==0:
        ...
        for a in plt_nums:
            ...
            if a!=0:
                ...
                plt.plot(x, h_analytic, "-", linewidth=1, label="h exact at "+str(int(t*10)/10) + "s")

            plt.plot(np.linspace(0,1000,nx), solver.vals[a,:,0]+dam_height, "--", linewidth=2, label="h at "+str(int(t*10)/10))
        ...
        for b in plt_nums:
            ...
            if b!=0:
                h_analytic,u_analytic = prob.get_analytic_solution(x,t)
                plt.plot(x, u_analytic, "-", linewidth=1, label="$u_x$ exact at "+str(int(t*10)/10))
            
            plt.plot(np.linspace(0,Lx,nx), solver.vals[b,:,1], "--", linewidth=2, label="$u_x$ at "+str(int(t*10)/10)+"s")
        plt.grid(True)
        plt.xlabel("x(m)")
